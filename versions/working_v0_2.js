...
function insertTask(description, user, sprintId) {
    return new Promise((resolve, reject) => {
        db.run(
            "INSERT INTO tasks (description, status, assigned_to, sprint_id) VALUES (?, ?, ?, ?)",
            [description, 'pendiente', user.username, sprintId],
            function (err) {
                if (err) {
                    reject(err);
                } else {
...
Debes determinar las acciones que el usuario desea realizar en relación con las tareas. Las acciones permitidas son:
  - "crear": para crear una tarea. Debes incluir la propiedad "description" que describa de forma clara y detallada la tarea.
  - "actualizar": para actualizar una tarea. Debes incluir "id" y "status".
  - "asignar": para asignar una tarea. Debes incluir "id" y "assigned_to".
  - "completar": para marcar una tarea como completada. Debes incluir "id".
...
responde con un array de 4 objetos, cada uno con "action": "crear" y una "description" que detalle cada paso o tarea necesario para implementar esa feature.
Ejemplos:
  - Entrada: "Crea una tarea para actualizar el sitio web"
    Respuesta: { "action": "crear", "description": "Actualizar el sitio web" }
  ...
  - Entrada: "Quiero que desgloses la feature de implementar un sistema de pago para recargar tarjetas NFC usando la tarjeta de crédito en 4 tareas"
    Respuesta: [
       { "action": "crear", "description": "Definir los requisitos funcionales y de seguridad para el sistema de pago NFC" },
       { "action": "crear", "description": "Diseñar la arquitectura técnica e integración con la pasarela de pago" },
       { "action": "crear", "description": "Implementar el módulo de recarga de tarjetas NFC y la validación de la tarjeta de crédito" },
       { "action": "crear", "description": "Realizar pruebas de integración y seguridad en el sistema de pago" }
    ]
...
`;
        ...
        const completion = await openai.createChatCompletion({
            model: "gpt-3.5-turbo",
            messages: [
                { role: "system", content: "Eres un asistente de project management muy preciso." },
        ...
        for (const command of commands) {
            if (command.action === "crear") {
                ...
                try {
                    const task = await insertTask(description, user, sprint.id);
